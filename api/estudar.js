...
function montarEmail(perfil, dataAlvo, entregas) {
    ...
    return `
        <div style="font-family:Arial,sans-serif;color:#25223b;max-width:760px;margin:auto">
            ...
            <div style="padding:24px;border:1px solid #e8e1ff;border-radius:0 0 20px 20px">
                <p>Estas são as tarefas confirmadas para <strong>${escaparHtml(dataAlvo)}</strong>.</p>
                ...
                <p style="color:#747087;font-size:13px;margin-top:20px">
                    Confira avisos importantes diretamente na Agenda e no Google Classroom da escola.
                </p>
            </div>
        </div>
    `;
...
export default async function handler(req, res) {
    try {
        ...
        if (!entregas.length) {
            return res.status(200).json({ enviado: false, motivo: "sem-entregas" });
        }

        const resumo = JSON.stringify(entregas);
        ...
        await enviarPeloResend(
            ...
            montarEmail(perfil, dataAlvo, entregas)
        );
