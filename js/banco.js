(function () {
    ...
    async function cadastrar(usuario, senha) {
        ...
        if (resposta.error) throw resposta.error;
        return { usuario: resposta.data.user, precisaConfirmarEmail: !resposta.data.session };
    }
    ...
    async function trocarSenhaObrigatoria(novaSenha) {
        if (typeof novaSenha !== "string" || novaSenha.length < 8) throw new Error("Use pelo menos 8 caracteres.");
        if (!cliente) throw new Error("Banco de dados não configurado.");
        const usuarioResposta = await cliente.auth.getUser();
        if (usuarioResposta.error) throw usuarioResposta.error;
        if (!usuarioResposta.data.user) throw new Error("Sua sessão expirou. Solicite um novo link de recuperação ou entre novamente.");
        if (emRecuperacaoSenha() && sessionStorage.getItem("malteriaRecuperacaoSenha") !== usuarioResposta.data.user.id) {
            throw new Error("Link de recuperação inválido ou expirado. Volte ao login e solicite um novo link.");
        }
    ...
    function emRecuperacaoSenha() {
        ...
        return parametros.get("recuperar-senha") === "1" ||
            Boolean(sessionStorage.getItem("malteriaRecuperacaoSenha")) ||
            /type=recovery/i.test(window.location.hash);
    }

    async function sair(opcoes = {}) {
        if (!cliente) return;
        if (opcoes.sincronizar !== false && !emRecuperacaoSenha()) {
            try { await salvarEstado(); } catch (erro) { console.warn("Não foi possível sincronizar antes de sair:", erro); }
        }
    ...
    async function salvarEstado() {
        if (!cliente || salvando || emRecuperacaoSenha()) return;
        salvando = true;
        try {
            const usuarioResposta = await cliente.auth.getUser();
    ...
    if (cliente) {
        cliente.auth.onAuthStateChange(function (evento, sessao) {
            if (evento === "SIGNED_OUT") sessionStorage.removeItem("malteriaRecuperacaoSenha");
            if (evento === "PASSWORD_RECOVERY" && sessao && sessao.user) {
                sessionStorage.setItem("malteriaRecuperacaoSenha", sessao.user.id);
